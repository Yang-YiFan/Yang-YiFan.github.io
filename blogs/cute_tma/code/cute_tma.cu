...
__global__ void cute_tma_multicast_kernel(__grid_constant__ const TmaLoad tma_load, GmemTensor gmem_tensor, SmemLayout smem_layout) {
    ...
    dim3 clusterDim = cute::cluster_shape();
    ...
    // 9. after this line, the TMA load is finished
    if (threadIdx.x == 0) {
        printf("block: (%d, %d, %d), cluster rank: %u, value: %f, %f\n", blockIdx.x, blockIdx.y, blockIdx.z, block_rank_in_cluster, float(smem_tensor(make_coord(0, 0))), float(smem_tensor(make_coord(0, 1))));
    }

    cute::cluster_sync();
}

template <typename T, int CTA_N, int CTA_K, int CLUSTER_SIZE>
void cute_host_multicast(T* data, int N, int K) {
    ...
    // 3. create the TMA object
    auto tma_load = make_tma_copy(SM90_TMA_LOAD_MULTICAST{}, gmem_tensor, smem_layout, Int<2>{});

    // 4. invoke the kernel using extensible launch interface
    cudaLaunchConfig_t config;
    cudaLaunchAttribute attrs[1];
    config.gridDim = dim3{N / CTA_N, K / CTA_K, CLUSTER_SIZE};
    config.blockDim = 32;
    ...
    attrs[0].val.clusterDim.z = CLUSTER_SIZE;
    config.attrs = attrs;
...
int main() {
    ...
    for (int i = 0; i < iter; i++) {
        ...
        //cute_host_multicast<cutlass::float_e4m3_t, TILE_N, TILE_K, CLUSTER_SIZE>(B.device_data(), N, K);
