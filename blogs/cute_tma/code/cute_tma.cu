...
int main() {
    ...
    for(int i = 0; i < N / PF_TILE_N; i++) {
        for(int j = 0; j < K / PF_TILE_K; j++) {
            B.at({i * PF_TILE_N, j * PF_TILE_K}) = cutlass::float_e4m3_t((float)i + j);
            B.at({i * PF_TILE_N, j * PF_TILE_K + 1}) = cutlass::float_e4m3_t((float)i - j);
        }
    }
    ...
    for (int i = 0; i < iter; i++) {
        ...
        //cute_host_prefetch<cutlass::float_e4m3_t, PF_TILE_N, PF_TILE_K>(B.device_data(), N, K);
    }
